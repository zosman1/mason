...
public class BasePhase extends SimState implements ECInterface
{
	...
	public void setTempDesCoefficient(final double val)
	{
		if(BasePhase.COEFF_LOWER_BOUND<=val&&val<=BasePhase.COEFF_UPPER_BOUND)
			parameters.initTempCoeff = val;
	}

	public Object domTempDesCoefficient() {  return new Interval(BasePhase.COEFF_LOWER_BOUND, BasePhase.COEFF_UPPER_BOUND); }

	public double getPortDesCoefficient() { return parameters.initPortCoeff; }
	public void setPortDesCoefficient(final double val)
	{
		if(BasePhase.COEFF_LOWER_BOUND<=val&&val<=BasePhase.COEFF_UPPER_BOUND)
			parameters.initPortCoeff = val;
	}

	public Object domPortDesCoefficient() { return new Interval(BasePhase.COEFF_LOWER_BOUND, BasePhase.COEFF_UPPER_BOUND); }

	public double getRiverDesCoefficient() { return parameters.initRiverCoeff; }
	public void setRiverDesCoefficient(final double val)
	{
		if(BasePhase.COEFF_LOWER_BOUND<=val&&val<=BasePhase.COEFF_UPPER_BOUND)
			parameters.initRiverCoeff = val;
	}

	public Object domRiverDesCoefficient() { return new Interval(BasePhase.COEFF_LOWER_BOUND, BasePhase.COEFF_UPPER_BOUND); }

	public double getElevDesCoefficient() {return parameters.initElevCoeff; }
	public void setElevDesCoefficient(final double val)
	{
		if(BasePhase.COEFF_LOWER_BOUND<=val&&val<=BasePhase.COEFF_UPPER_BOUND)
			parameters.initElevCoeff = val;
	}

	public Object domElevDesCoefficient() { return new Interval(BasePhase.COEFF_LOWER_BOUND, BasePhase.COEFF_UPPER_BOUND); }
     ...
     *
     */
    public BasePhase(final long seed) { this(seed, null); }

    public BasePhase(final long seed, final String[] args)
    {
    	super(seed);
     ...
     */
    public double[] extractSpikinessDataFromGrid(final DoubleGrid2D theGrid)
    {
        // Fill the model data from the desirability grid
        final double[] theData = new double[map.canadaCells.size()];
        double minDataVal = Double.POSITIVE_INFINITY;
        int index = 0;
        for (final Cell c : map.canadaCells)
        {
            final double val = map.popGrid.field[c.x][c.y];
            theData[index++] = val;
     ...
     * I created this function as a default for people to build from.
     */
    public double[] extractSmoothedDataFromGrid(final DoubleGrid2D theGrid)
    {
        final double[] thePop = new double[map.canadaCells.size()];
        double theMin = Double.POSITIVE_INFINITY;
        int i = 0;
        for (final Cell cell : map.canadaCells)
        {
            final double val = theGrid.field[cell.x][cell.y];
            thePop[i] = val;
        ...
        final double KL_adjust = 1.0 / (thePop.length * 1000);
        for(i = 0; i < thePop.length; i++)
        	{ thePop[i] += KL_adjust; }
    ...
    public double calcInvertedKolmogorovSmirnovStatistic(final double[] a, final double[] b, final MersenneTwisterFast random)
    {
        final KolmogorovSmirnovTest ksTest = new KolmogorovSmirnovTest(new MersenneTwisterFastApache(random));
     ...
     *
     */
    public double klDivergence(final double[] p1, final double[] p2)
    {
        double klDiv = 0.0;
    ...
    public double calcKLDivergenceMetric(final double[] a, final double[] b)
    {
        final double klDivAB = klDivergence(a, b);
        final double klDivBA = klDivergence(b, a);
        final double klDiv = (Math.abs(klDivAB) + Math.abs(klDivBA))/2;   // range between 0 and ~7, with 0 being better
    ...
    {
        final double[] modelPop = this.gimmeSmoothedModelPop();
        final double[] empiricalPop = this.gimmeSmoothedEmpiricalPop();
        //double smoothFitness = calcInvertedKolmogorovSmirnovStatistic(targetPop, modelPop, random);
        final double smoothFitness = 1.0/(1.0 + calcKLDivergenceMetric(empiricalPop, modelPop));  // range 0 to 1
    ...
    {
        final double[] empiricalPop = this.gimmeEmpiricalPop();
        final double[] modelPop = this.gimmeModelPop();
        final double invKSStatistic = calcInvertedKolmogorovSmirnovStatistic(empiricalPop, modelPop, random);
    ...
    {
        final double smoothedFitness = calcSmoothedFitness();
        final double spikinessFitness = calcSpikinessFitness();
        final double totalFitness = smoothedFitness * spikinessFitness;
    ...
    public double[] gimmeEmpiricalPop()
    {
        ...
        final double[] empiricalPop = Utility.convertToDoubles(map.getPopGridData(1));
	...
	protected void inspectPopGrid()
	{
		...
		for(int i = 0;i<smoothedTargetPopGrid.getWidth();++i)
		{
			for(int j = 0;j<smoothedTargetPopGrid.getHeight();++j)
			{
				if(smoothedTargetPopGrid.get(i, j)!=Map.MISSING&&map.getNationGrid().get(i, j)==Map.CANADA_CODE)
					sumSmoothed += smoothedTargetPopGrid.get(i, j);
			}
		}
