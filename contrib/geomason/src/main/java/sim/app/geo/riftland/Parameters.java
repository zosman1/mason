	...
	private void checkConstraints() {
		if (households.meanHouseholdSize < 1)
			throw new IllegalStateException(
					"Parameters: meanHouseHoldSize < 1, but must be >= 1.");
		if (households.laborProductionRate <= 0)
			throw new IllegalStateException(
					"Parameters: laborProductionRate must be greater than 0.");
		if (herding.visionAndMovementRange <= 0)
			throw new IllegalStateException(
					"Parameters: visionAndMovementRange must be >= 1.");
		if (world.numInitialWateringHoles < 0)
			throw new IllegalStateException(
					"Parameters: numInitialWateringHoles must be >= 0.");
		if (vegetation.minVegetationKgPerKm2 <= 5)
			throw new IllegalStateException(
					"Parameters: vegetation.minVegetationKgPerKm2 must be > 5.");
	}

	public static ParameterDatabase openParameterDatabase(String relative,
			Class reproduction) {
...
//                    parameters = new ParameterDatabase(parameterDatabaseFile.getAbsoluteFile());
					// System.err.println(parameterDatabaseFile.getAbsoluteFile() + " that
					// b path " + Arrays.toString(args));
	...
	private static File getFile(String nodesFilename) throws IOException {
		...
		try {
			...
			File targetFile = new File("./shapeFiles/"
					+ nodesFilename.split("/")[nodesFilename.split("/").length - 1]);
...
//        system.setWriteStats(returnBooleanParameter(parameterDB, "OutputStats", false));
		system.setOutputFilename(
				returnStringParameter(parameterDB, "OutputFilename", null));
		system.setEthnicityOutputFilename(
				returnStringParameter(parameterDB, "EthnicityFilename", null));
		system.setDisplacedOutputFilename(
				returnStringParameter(parameterDB, "DisplacementFilename", null));
		system.setDisplacedDetailsOutputFilename(
				returnStringParameter(parameterDB, "DisplacementDetailsFilename", null));
		system.setNumthreads(returnIntParameter(parameterDB, "Numthreads",
		...
		world.setInitTruePopulationPortion(
				returnDoubleParameter(parameterDB, "InitTruePopulationPortion", 1.0));

		world.setOnlyOneHousehold(
				returnBooleanParameter(parameterDB, "OnlyOneHousehold", false));
		...
		world.setWeatherSequence(
				returnStringParameter(parameterDB, "WeatherSequence", null));
		...
		world.setScheduleMediator(returnBooleanParameter(parameterDB, "ScheduleMediator",
				world.isScheduleMediator()));

		world.setPermanentWaterThreshold(returnDoubleParameter(parameterDB,
				"PermanentWaterThreshold", world.getPermanentWaterThreshold()));

		world.setWaterHoleCounterFlag(
				returnBooleanParameter(parameterDB, "WaterHoleCounterFlag", false));
		...
		households.setInitialEndowmentInYears(returnDoubleParameter(parameterDB,
				"InitialEndowmentInYears", households.getInitialEndowmentInYears()));

		households.setLaborProductionRate(returnDoubleParameter(parameterDB,
				"LaborProductionRate", households.getLaborProductionRate()));

		households.setCanFarm(
				returnBooleanParameter(parameterDB, "CanFarm", households.canFarm()));
		households.setCanHerd(
				returnBooleanParameter(parameterDB, "CanHerd", households.canHerd()));

		// Farming parameters
		farming.setPlantingThreshold(returnDoubleParameter(parameterDB,
				"PlantingThreshold", farming.getPlantingThreshold()));

		farming.setKgOfGrainPerPersonDay(returnDoubleParameter(parameterDB,
				"KgOfGrainPerPersonDay", farming.getKgOfGrainPerPersonDay()));

		farming.setMaxWorkableLandPerFarmer(returnDoubleParameter(parameterDB,
				"MaxWorkableLandPerFarmer", farming.getMaxWorkableLandPerFarmer()));

		farming.setFarmIntensificationExponent(returnDoubleParameter(parameterDB,
				"FarmIntensificationExponent", farming.getFarmIntensificationExponent()));

		farming.setMaxMoveDistanceForNewHousehold(
				returnIntParameter(parameterDB, "MaxMoveDistanceForNewHousehold",
						farming.getMaxMoveDistanceForNewHousehold()));

		farming.setFarmingRestartProbabilityExponent(
				returnDoubleParameter(parameterDB, "FarmingRestartProbabilityExponent",
						farming.getFarmingRestartProbabilityExponent()));

		// Herding: Physiology parameters
		herding.setTLUFoodConsumptionRate(returnDoubleParameter(parameterDB,
				"TLUFoodConsumptionRate", herding.getTLUFoodConsumptionRate()));

		herding.setTLUFoodMetabolismRate(returnDoubleParameter(parameterDB,
				"TLUFoodMetabolismRate", herding.getTLUFoodMetabolismRate()));

		herding.setTLUWaterMetabolismRate(returnIntParameter(parameterDB,
				"TLUWaterMetabolismRate", herding.getTLUWaterMetabolismRate()));

		herding.setTLUMaxDaysWithoutFood(returnIntParameter(parameterDB,
				"TLUMaxDaysWithoutFood", herding.getTLUMaxDaysWithoutFood()));

		herding.setTLUMaxDaysWithoutWater(returnIntParameter(parameterDB,
				"TLUMaxDaysWithoutWater", herding.getTLUMaxDaysWithoutWater()));
		...
		herding.setHerdIdealBirthProbability(returnDoubleParameter(parameterDB,
				"HerdIdealBirthProbability", herding.getHerdIdealBirthProbability()));

		herding.setFoodCostOfMovement(returnDoubleParameter(parameterDB,
				"FoodCostOfMovement", herding.getFoodCostOfMovement()));

		herding.setWaterCostOfMovement(returnDoubleParameter(parameterDB,
				"WaterCostOfMovement", herding.getWaterCostOfMovement()));

		// Herding: Behavior parameters
		herding.setHerdingFoodWeight(returnDoubleParameter(parameterDB,
				"HerdingFoodWeight", herding.getHerdingFoodWeight()));

		herding.setHerdingWaterWeight(returnDoubleParameter(parameterDB,
				"HerdingWaterWeight", herding.getHerdingWaterWeight()));

		herding.setHerdingDistanceWeight(returnDoubleParameter(parameterDB,
				"HerdingDistanceWeight", herding.getHerdingDistanceWeight()));

		herding.setHerdingHomeWeight(returnDoubleParameter(parameterDB,
				"HerdingHomeWeight", herding.getHerdingHomeWeight()));

		herding.setReturnHomeCheckInterval(returnIntParameter(parameterDB,
				"ReturnHomeCheckInterval", herding.getReturnHomeCheckInterval()));

		// Herding: Conversions parameters
		herding.setTLUPerPFD(
				returnIntParameter(parameterDB, "TLUPerPFD", herding.getTLUPerPFD()));

		herding.setPFDPerConsumedTLU(returnDoubleParameter(parameterDB,
				"PFDPerConsumedTLU", herding.getPFDPerConsumedTLU()));

		herding.setMaxTLUsPerHerder(returnIntParameter(parameterDB, "MaxTLUsPerHerder",
				herding.getMaxTLUsPerHerder()));

		herding.setHerdVegetationThreshold(returnDoubleParameter(parameterDB,
				"HerdVegetationThreshold", herding.getHerdVegetationThreshold()));

		herding.setVisionAndMovementRange(returnIntParameter(parameterDB,
				"VisionAndMovementRange", herding.getVisionAndMovementRange()));

		herding.setMaxNearbyWaterHoles(returnIntParameter(parameterDB,
				"MaxNearbyWaterHoles", herding.getMaxNearbyWaterHoles()));

		herding.setHerdSplitThreshold(returnIntParameter(parameterDB,
				"HerdSplitThreshold", herding.getHerdSplitThreshold()));

		herding.setHerdingRestartProbabilityExponent(
				returnDoubleParameter(parameterDB, "HerdingRestartProbabilityExponent",
						herding.getHerdingRestartProbabilityExponent()));

		herding.setPlentyOfWaterInTLUDays(returnIntParameter(parameterDB,
				"PlentyOfWaterInTLUDays", herding.getPlentyOfWaterInTLUDays()));

		// Herding: other parameters
		herding.setMaxTrailLength(returnIntParameter(parameterDB, "MaxTrailLength",
				herding.getMaxTrailLength()));

		herding.setUseHerdingRuleBased(returnBooleanParameter(parameterDB,
				"HerdUseRuleBased", herding.isUseHerdingRuleBased()));
	}
	 ...
	 *
	 *         XXX consider moving this to a separate utility class; or changing
	 *         MASON
	 *         to accept mere strings
	 *
	 */
	public int returnIntParameter(ParameterDatabase paramDB, String parameterName,
			int defaultValue) {
		return paramDB.getIntWithDefault(new Parameter(parameterName), null,
				defaultValue);
	}
	 ...
	 *
	 */
	public boolean returnBooleanParameter(ParameterDatabase paramDB, String parameterName,
	 ...
	 *
	 */
	double returnDoubleParameter(ParameterDatabase paramDB, String parameterName,
			double defaultValue) {
		return paramDB.getDoubleWithDefault(new Parameter(parameterName), null,
				defaultValue);
	}
	 ...
	 *
	 */
	String returnStringParameter(ParameterDatabase paramDB, String parameterName,
			String defaultValue) {
		return paramDB.getStringWithDefault(new Parameter(parameterName), null,
				defaultValue);
	}
	// </editor-fold>
	...
	public class SystemParams {
		...
		private int numthreads = Runtime.getRuntime().availableProcessors(); // Number of
																				// threads
																				// to
																				// spawn
																				// for
																				...
																				// in
																				// parallel
		private boolean runExpensiveAsserts = false; // Asserts that are known to be
														// expensive will only be checked
														// if this is true and asserts are
														// enabled.
		...
		public void setLoggerLevel(final String levelString) {
			...
			try {
				...
				System.err.println("Bogus logging string: " + levelString
	...
	public class WorldParams {
		...
		/**
		 * If true, then a Mediator object is scheduled to run.
		 * <p>
		 * Can be set by
		...
		/**
		 * The percentage of the true population to place in a parcel
		 * <p>
		 * In range
		 * (0,1].
		 *
		...
		/**
		 * Comma-seperated sequence of weather years to run, after which, the years loop
		 * as normal.
		 */
...
//        }
	...
	public class VegetationParams {
		/**
		 * What is this? Where is it used? < explains magic number for
		 * theoreticalMaxVegetationKgPerKm2
		 *
		 ...
		 */
		private double theoreticalMaxVegetationKgPerHectare = 1.1 * 1000; // kgDM/ha=
																			// tonsDM/ha *
																			// kg/ton
		private double theoreticalMaxVegetationKgPerKm2 = theoreticalMaxVegetationKgPerHectare
				* 100; // kg/km2= tons DM/ha * kg/ton * ha/km2

		/**
		 * Minimum rainfall needed for growth (per day) (alpha in the MatLab GA code).
		 * Set by the GA.
		 */
		// private double growthThreshold = 4.237; // value produced by GA with no
		// herders
		 ...
		 * grow fast enough).
		 */
		private double minVegetationKgPerKm2 = minVegRatio
				* theoreticalMaxVegetationKgPerKm2;
		 ...
		 * shrink fast enough).
		 */
		private double maxVegetationKgPerKm2 = maxVegRatio
				* theoreticalMaxVegetationKgPerKm2;
		...
		public void setTheoreticalMaxVegetationKgPerHectare(
				double theoreticalMaxVegetationKgPerHectare) {
		...
		public void setTheoreticalMaxVegetationKgPerKm2(
				double theoreticalMaxVegetationKgPerKm2) {
		...
		public void setMinVegetationKgPerKm2(double minVegetationKgPerKm2) {
			assert (minVegetationKgPerKm2 > 5); // this is necessary for visualization
												// purposes
	...
	public class FarmingParams {
		...
		/**
		 * amount of farm land per farmer at which productivity peaks (currently
		 * 1ha/farmer)
		 */
		...
		/**
		 * This is the radius of maximum Moore neighborhood that we would like to search
		 * for finding a suitable parcel to locate new farm.
		 */
		...
		public void setFarmingRestartProbabilityExponent(
				double farmingRestartProbabilityExponent) {
		...
		public void setMaxMoveDistanceForNewHousehold(
				int maxMoveDistanceForNewHousehold) {
	...
	public class HerdingParams {
		...
		/**
		 * This determines which HerdingParams subclass is used for herder behavior.
		 * <p>
		...
		// Not sure about above comment, I think it is the base amount a TLU needs to
		// eat each day -- TIM
		 ...
		 * (risks death?)
		 */
		private double TLUFoodStress = 0.5 * TLUFoodMetabolismRate
				* TLUMaxDaysWithoutFood;
		/** Number of days an animal can go without water before dying */
		private int TLUMaxDaysWithoutWater = 6; // 2-3 to "less than a week"

		/**
		 * The amount of water (in liters) metabolized each day per TLU. FAO study has
		 * cattle
		 * drinking 5 liters/day in the wet season (due to water in vegetation, and 27
		 * liters/day
		 * in the dry season.
		 ...
		 * minWater is 0.
		 */
		private double TLUWaterMax = 1.25 * TLUWaterMetabolismRate
				* TLUMaxDaysWithoutWater;
		/**
		 * The amount of water (in its belly) below which an animal becomes
		 * stressed (risks death?)
		 *
		 */
		private double TLUWaterStress = 0.5 * TLUWaterMetabolismRate
				* TLUMaxDaysWithoutWater;
		 ...
		 * on 10.25 kgDM/day consumption
		 * <p>
		 * XXX what is the reference for this?
		 */
		private double herdVegetationThreshold = 1025;

		/**
		 * XXX How far a unit can "see"?
		 * <p>
		 * XXX NOTE THIS <B>ALSO</B> DENOTES
		 * MAXIMUM MOVEMENT PER DAY!!
		 */
		private int visionAndMovementRange = 10;
		/**
		 * Max number of waterholes stored in each WaterHole's nearbyWateringHoles
		 * cache.
		 */
		...
		/**
		 * how much weight we should give to returning home to plant when deciding where
		 * to go
		 */
		...
		/**
		 * Amount of water in TLU days that a waterhole must have to be considered
		 * "plenty".
		 * Above "plenty", all waterholes are considered equal for herd movement
		 * purposes.
		 */
		private int plentyOfWaterInTLUDays = 10000; // 10 herds of 100 cows drinking for
													// 10 days

		/**
		 * Controls how quickly a household will give up on herding (larger is more
		 * persistent)
		 */
		...
		public void setHerdingRestartProbabilityExponent(
				double herdingRestartProbabilityExponent) {
		...
		/**
		 * Amount of water a WaterHole ought to have (in liters) to be considered to
		 * have
