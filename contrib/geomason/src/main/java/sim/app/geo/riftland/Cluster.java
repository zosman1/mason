...
public class Cluster // implements Steppable
{
	 ...
	 */
	private boolean clusterFlag = true; // true=read ClusterControl file, false=generate
										// randomly
	...
	public Cluster(Parameters params, boolean x, final String dataPath,
	...
	private void setClusterMultipliers(final String datapath,
			...
			try {
				...
				for (int year = 0; year < numYears; year++) {
					...
					for (int cluster = 0; cluster < numClusters; cluster++) {
						clusterMultipliers[cluster][year] = Double
								.parseDouble(tokens[cluster + 1]);
					}
				}
