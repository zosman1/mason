...
public class Map
{
    ...
    public static final int REGION_HEIGHT = Map.REGION_WIDTH;
    ...
    public Map(final Parameters params, final MersenneTwisterFast random_)
    {
        nationGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        provinceGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        megaCellGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        megaCellSignGrid = new Continuous2D(1.0, Map.GRID_WIDTH, Map.GRID_HEIGHT);
        popGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        cultureGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        landCoverGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        nppGrid = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        tempRawToAdd = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // The most recent temperatures
        tempRaw = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // The most recent temperatures
        tempRawToSubtract = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // Remove from moving average
        tempRawMovingAverage = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // Averaged temps over some period
        tempRawMovingAverageC = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // Averaged temps over some period in Celsius
        tempTemp = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // Used to temporarily store temperature data
        tempDes = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);  // derived from tempRawMovingAverage
        tempVariance = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        latGrid = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        lonGrid = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        portRaw = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        portDes = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        riverRaw = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        riverDes = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        elevRaw = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        elevDes = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        initDesGrid = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        cellGrid = new ObjectGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        coastalGrid = new DoubleGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        initPopRegionGrid = new IntGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        canadaCells = new ArrayList<Cell>(91777);    // how many Canadian cells there are

        cities = new SparseGrid2D(Map.GRID_WIDTH, Map.GRID_HEIGHT);
        ...
        try {
            ...
            nationGrid.replaceAll(Map.MISSING, 0);
            nationGrid.replaceAll(124, Map.CANADA_CODE);  // Canada

            coastalGrid.replaceAll(Map.MISSING, 0);
            ...
            weatherIO.setNationGrid(nationGrid);
...
*/
        ...
        tempDes.replaceAll(Map.MISSING, Double.NEGATIVE_INFINITY);    // TODO: We should find a better way
        portDes.replaceAll(Map.MISSING, Double.NEGATIVE_INFINITY);
        riverDes.replaceAll(Map.MISSING, Double.NEGATIVE_INFINITY);
        elevDes.replaceAll(Map.MISSING, Double.NEGATIVE_INFINITY);

        final int waterCount = initializeDesirabilityGrid();
...
*/
    ...
    {
        final double tWeight = parameters.initTempCoeff;
        final double pWeight = parameters.initPortCoeff;
        final double rWeight = parameters.initRiverCoeff;
        final double eWeight = parameters.initElevCoeff;

        int waterCount = 0;

        for(int y = 0; y < Map.GRID_HEIGHT; y++) {
            for(int x = 0; x < Map.GRID_WIDTH; x++) {
                if (nationGrid.get(x, y)==Map.CANADA_CODE) {
                    // exclude cells with land cover of 0 (water), unless people really live there
                    if ((nppGrid.field[x][y] < 0) && (popGrid.field[x][y] <= 0)) {
                        waterCount++;
                        continue;
                    }
                    final double totDes = calculateInitialDesirability(x, y, tWeight, pWeight, rWeight, eWeight);
                    if (Double.isInfinite(totDes) || Double.isNaN(totDes))
                        continue;
                    initDesGrid.set(x, y, totDes);
                    final Cell c = new Cell(x, y, nationGrid.get(x, y), provinceGrid.get(x,y), tempDes.get(x, y), portDes.get(x, y), elevDes.get(x, y), riverDes.get(x, y), totDes, popGrid.get(x, y));
                    cellGrid.set(x, y, c);
                    canadaCells.add(c);
                    indexMap.put(c,canadaCells.size()-1);
                }
                else {
    ...
    public void updateDesirabilityGrid()
    {
        ...
        for(final Cell c : canadaCells)
        {
            final double totDes = calculateInitialDesirability(c.x, c.y, tWeight, pWeight, rWeight, eWeight);
            c.initDes = totDes;
            //c.tempDes = 0;
            //c.portDes = 0;
            //c.elevDes = 0;
    ...
    private void initializeMegaCellGrid() {
    	...
    	final int megaSize = 30;
    	final int height = megaCellGrid.getHeight();
    	final int width = megaCellGrid.getWidth();
    	int counter = 0;
    	...
    	for(int i = 0;i<height;i+=megaSize) {
    		for(int j = 0;j<width;j+=megaSize) {
    			...
    			for(int ii = 0;ii<megaSize;++ii) {
    				for(int jj = 0;jj<megaSize;++jj) {
    					if(ii+i<height&&jj+j<width) {
    						...
    						final Cell cell = (Cell) cellGrid.get(jj+j, ii+i);
    						if(cell!=null)
    						{
    			...
    			{
    				final MegaCellSign sign = new MegaCellSign(counter, new Double2D(j+megaSize/2,i+megaSize/2));
    				megaCellTable.put(counter, sign);
    				megaCellSignGrid.setObjectLocation(sign, new Double2D(j+megaSize/2,i+megaSize/2));
    ...
    public void calcDistanceFromIdeal(final DoubleGrid2D srcGrid, final DoubleGrid2D destGrid, final double ideal)
    {
        //System.out.println("calcDistanceFromIdeal");
        if(destGrid != srcGrid)
            destGrid.setTo(srcGrid);

        final int width = srcGrid.getWidth();
        final int height = srcGrid.getHeight();
        double val = 0.0;
...
*/
    ...
    private double[] zscoreValue(final DoubleGrid2D desGrid, final DoubleGrid2D srcGrid) {
        ...
        for (int y = 0; y < Map.GRID_HEIGHT; y++) {
            for (int x = 0; x < Map.GRID_WIDTH; x++) {
                if (nationGrid.get(x, y) == Map.CANADA_CODE) {
                    final double val = srcGrid.field[x][y];
                    sum += val;
        ...
        final double mean = sum / counter;
        final double sd = Math.sqrt(sumOfSquares / counter - mean * mean);
    ...
    private Int2D findNearestPointToLatitude(final double latitude)
    {
        ...
        for (int y = 0; y < Map.GRID_HEIGHT; y++)
        {
            for (int x = 0; x < Map.GRID_WIDTH; x++)
            {
                final double distance = Math.abs(latGrid.field[x][y] - latitude);
        ...
        final double centerX = 427;
        final double centerY = 481;
        final Int2D point = new Int2D(nearestX, nearestY);
        final double gridDistance = point.distance(centerX, centerY);
    ...
    public int countMatchingCells(final IntGrid2D grid, final int value)
    {
        int count = 0;
    ...
    public void updateTotalDesirability(final Parameters params)
    {
        double totDes;
    ...
    private double[] getCanadianSubset(final DoubleGrid2D grid)
    {
        final double[] data = new double[canadaCells.size()];
        int index = 0;
        for (final Cell c : canadaCells)
...
*/
        ...
        final double[] varData = getCanadianSubset(tempVariance);
...
//        tempRawMovingAverageC.setTo(0.0);
    ...
    {
        final int[] data = new int[canadaCells.size()];
        int index = 0;
        for (final Cell c : canadaCells)
    ...
    public int[] getPopGridData(final int minVal)
    {
        final ArrayList<Integer> sizes = new ArrayList<Integer>();

        for (final Cell cell : canadaCells)
                {
            if (cell.empPop > 0)
        ...
        final int[] a = new int[sizes.size()];
    ...
    public double[] getDoublePopGridData(final double minVal)
    {
        final ArrayList<Integer> sizes = new ArrayList<Integer>();
        ...
        final double[] a = new double[sizes.size()];
        for (int i = 0; i < a.length; i++)
            a[i] = sizes.get(i);
     ...
     */
    public double calculateInitialDesirability(final int x, final int y, final double tWeight, final double pWeight, final double rWeight, final double eWeight) {
    ...
    public void calcInitialTempMovingAverage(final Parameters parameters)
    {
    ...
    public int updateTemperatures(final NorthLandsMovement model, final int seasonIndex)
    {
        ...
        if (parameters.tempAdjustStart<=model.schedule.getTime()) {tempRawToSubtract = tempRawToSubtract.add(model.getMeanTempAdjust() * Math.max(seasonToRemove, 0.0) / seasonsPerYear);}
        ...
        if (parameters.tempAdjustStart<=model.schedule.getTime()) {tempRawToAdd = tempRawToAdd.add(model.getMeanTempAdjust() * seasonIndex / seasonsPerYear);}
...
*/
    ...
    private DoubleGrid2D adjustVariance(final DoubleGrid2D srcGrid,
            final double stdevTempAdjust) {
        double sum = 0.0;
        int counter = 0;
        for(int y = 0; y < Map.GRID_HEIGHT; y++) {
            for(int x = 0; x < Map.GRID_WIDTH; x++) {
    ...
    private void updateWithMiuAndSigma(final DoubleGrid2D desGrid,
            final DoubleGrid2D srcGrid, final double[] params) {
        //System.out.println("update temperature grid");
        //System.out.println(params[0]);
        //System.out.println(params[1]);

        final double mean = params[0];
        final double sd = params[1];
    ...
    public DoubleGrid2D loadDoubleGrid2D(final String filename)
    {
        ...
        try
        {
            ...
            final InputStream fileIn = getClass().getClassLoader().getResourceAsStream(filename);
     ...
     */
    public void populateGrid(final String filename, final IntGrid2D grid) throws Exception {
//        InputStream is = getClass().getClassLoader().getResourceAsStream(filename);
     ...
     */
    public void populateGrid(final String filename, final DoubleGrid2D grid) throws Exception {
//        InputStream is = getClass().getClassLoader().getResourceAsStream(filename);
        final FileInputStream is = new FileInputStream(filename);
    ...
    public void remapCountryCodes(final IntGrid2D grid) {
        final boolean [] bools = new boolean [900];   // Country codes go into the 800s
        for(int i=0;i<grid.getHeight();i++) {
            for(int j=0;j<grid.getWidth();j++) {
//                bools[grid.get(j, i)]= true;
                final int val = grid.get(j, i);
                if (val < 0)
                {
        ...
        int count = 0;
        final int [] numbers = new int[bools.length];
    ...
    public Double2D getLonLat(final int x, final int y)
    {
